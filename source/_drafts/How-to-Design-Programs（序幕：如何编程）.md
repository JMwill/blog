...
图七的程序由四个定义组成：一个函数定义以及三个常数定义。数字100以及60一共出现了两次，一次是在一次是赋值给WIDTH时，一次是赋值给HEIGHT时。你可能也注意到了，picture-of-rocket.v4函数用了h作为参数名而不是height。严格来说，这一个改变是不必要的，因为DrRacket并不会混淆height跟HEIGHT，但是为了不混淆我们自己，所以这样做了。

当DrRacket运行`(animate picture-of-rocket.v4)`时，它在每一次遇到这些常量名时都会相应地将HEIGHT替换成60，WIDTH替换成100，ROCKET替换成图片。要体验真正的程序员的快乐，将HEIGHT的下一个值由60改为400然后点击RUN，你可以看到，火箭下降并登陆在一个100乘400的场景中。一个小小的改变就完成了全部。

用时髦的说法，你刚刚经历了你第一个程序的重构。每一次你重新组织你的程序用于为未来可能会有的变更做准备，你都是在重构你的程序。将这个放到你的简历中，这听起来十分棒，你未来的雇主也许会喜欢看到这些流行语，即使这些并没有使你成为一个好的程序员。然而好的程序员从来不会让一段程序存在有三个相同的表达式：

`（- HEIGHT (/ (image-height ROCKET) 2))`

每一次你的朋友以及同事在看这段程序的时候，他们都需要明白这个表达式到底在计算什么，其实是指画板的顶部与火箭中心点之间的距离。每一次DrRacket计算这个表达式的值的时候，它都要执行三步：（1）确定图片的高度；（2）将它除以2；然后（3）用HEIGHT来减去这个值。然后，每一次用的都是相同的数字。

这次的观察提醒我们要引入多一个定义到程序中：
...
现在，将程序中剩下的表达式`(- HEIGHT (/ (image-height ROCKET) 2))`替换为ROCKET-CENTER-TO-TOP。你可能会想知道这个定义是应该放在HEIGHT定义的上面还是下面。更一般来说，你应该想知道定义的先后顺序究竟（对程序）有没有影响。答案是对于常量的定义，先后顺序是有影响的；对于函数定义就没有。只要DrRacket遇到常量定义，它马上确定表达式的值并将其与名字联系起来。例如，
...
会导致DrRacket抛出“CENTER is used before its definition,”的错误，当它遇到HEIGHT的定义时。与之相反是：
...
会正确地工作。首先，DrRacket将100跟CENTER联系起来。然后，它计算`(* 2 CENTER)`，并得到200。最后，DrRacket将200跟HEIGHT联系起来。
...
现在我们可以再次修复picture-of-rocket。不是简单地使用t来替换height，函数可以用`(distance t)`来计算火箭下降的速度有多快。最终的程序如图九显示的那样。它包含有两个函数定义：picture-of-rocket.v6以及distance。剩余的常量定义让函数定义更可读更易修改。一如既往地，你可以用[animate][animate]运行程序。

`(animate picture-of-rocket.v6)`

于前一版本的picture-of-rocket相比，这一版本显示一个程序可能会包含有几个相互联系的函数定义。再然后，即使是第一个也都会用+号以及/号你只需要将这些认为是BSL內建的就可以了。

当你成为一个真正的程序员，你会明白，一个程序包含有很多的函数定义，以及很多的常量定义。你也可以看到函数之间始终相互参照，你真正需要练习的是如何组织它们来让你即使已经将程序完成了一个月后还能很容易看懂它。毕竟，你的旧版本程序或者其他人的旧版本程序都会想要（在之后）作出改变。如果你无法明白程序是如何组织的，那么任何的小任务都会让你经历一个艰难的时刻。否则，你最好知道应该要知道的。

---

##  你现在是一个程序员了

在前一节结束时声称你是一个程序员可能会让你很意外，但这是真的，你已经知道了所有关于BSL的构成法。你知道了程序是关于数字、字符串、图像、以及任何其他你选择的程序语言支持处理的数据的运算，你知道了程序是由函数以及常量定义来组成的。你知道这些是因为我们在最后告诉了你程序都是关于如何恰当地组织这些定义。最后但是同样重要的是，一直到DrRacket以及教学包支持大量其他的函数以及DrRacket的帮助页面解释了这些程序是干什么用的。

你可能会想，对于编写程序你依然知道得不够多，你还不知道如何对按键输入、鼠标点击以及其他的事件作响应。事实证明你是知道的，除了[animate][animate]函数，_2htdp/universe_ 提供了其他的函数用来将你的程序跟键盘，鼠标，时钟或其他计算机中运动的部件进行连接。实际上，它甚至支持编写一个程序用来将你的电脑跟世界上任何人的电脑进行连接。所以这不是一个真正的问题。

简而言之，你已经看到过了几乎全部的程序整合机制。如果你看过所有可用的程序，你可以编写来玩有趣的电脑游戏，运行仿真程序，或者跟踪商业账户。问题是这些是否真的意味着你是一个程序员？你真的是吗？
...
当你随机在某个书店货架上浏览“编程”的书籍时，你会看到很多书都承诺当场就能让你成为一个程序员。现在你已经以你的工作方式完成了第一个例子，然而，你可能也意识到这不可能是真的。

获得编程的机械技能——学习编写电脑能懂的表达式，开始了解那些函数以及库是可用的，或类似的活动，对于真正的编程是没有任何用处的。作出这样的声明就像是教你通过从字典上记住上千个单词以及一些语法规则来学习外语。

良好的编程不仅仅是习得语言的机制。更重要的是，需要铭记程序员创建程序是为了让其他人在未来能读懂它们。一个好的程序能反映出问题的陈述，以及它的重要概念。它提出了关于它计算什么的一个简明的描述，用例子说明它的目的，以及它是如何与问题相关的。例子确保了未来的读者知道你的程序怎么样工作以及为什么能工作。简单地说，好的程序是关于（如何）系统解决问题以及用代码传达想法的。最好的是，这种编程方法实际上使得编程对所有人都可以访问。因此它一次服务了两个主人。

这本书的剩下内容所有都是关于这些东西；书中很少的内容是关于DrRacket，BSL，或者库的技法。这本书向你展示一个程序员是如何想问题的。以及你甚至会学习应用这种方法来解决实际生活中的问题，例如：医生、记者、律师以及工程师的工作。

噢，顺便说一下，这本书剩下的内容用的语调更接近于严肃的文本，而不是像这一章序幕那样。请继续享用！
